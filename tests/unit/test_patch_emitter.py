...
def test_candidate_from_patch_text_rejects_non_diff_text() -> None:
    with pytest.raises(ValueError):
        _ = candidate_from_patch_text(patch_text="not a diff artifact")


def test_resolve_patch_candidate_fails_closed_for_invalid_cached_vcr_text() -> None:
    tenant_id = "t1"
    repo_id = "r1"
    plan_id = "p1"
    step_id = "s1"
    ...
    manifest = RunManifest(
        ...
        ir_sha256="0" * 64,
        replay_mode=replay_mode,
        llm_vcr={expected_prompt_key: "```diff\n--- a/x\n+++ b/x\n```"},
    )
